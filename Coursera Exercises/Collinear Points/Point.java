...
public class Point implements Comparable<Point> {
    ...
    public static void main(String[] args) {
        ...
        assert a.slopeTo(d) == 2;
        assert a.slopeTo(b) == Double.POSITIVE_INFINITY;
        assert a.slopeTo(a) == Double.NEGATIVE_INFINITY;
        assert a.slopeTo(c) == 0;

        Comparator<Point> aComparator = a.slopeOrder();

        assert aComparator.compare(a, b) < 0;
        assert aComparator.compare(b, c) > 0;
        assert aComparator.compare(c, d) < 0;
        assert aComparator.compare(d, c) > 0;
        assert aComparator.compare(b, d) > 0;
    }
}
